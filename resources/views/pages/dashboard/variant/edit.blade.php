    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div>
                ...
                @endif
                <form action="{{ route('dashboard.variant.update', $variant->id) }}" method="POST" enctype="multipart/form-data" class="w-full">
                    @csrf
                    @method('PUT')
                        <div class="flex flex-wrap -mx-3 mb-6">
                            <div class="w-full px-3 mb-8">
                                ...
                                <input value="{{ $variant->name }}" name="name" id="grid-last-name" placeholder="Category Name" type="text" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                            </div>
                            <div class="w-full px-3 mb-8">
                                <label for="grid-last-price" class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-3">
                                    Price
                                </label>
                                <input value="{{ $variant->price }}" name="price" id="grid-last-price" placeholder="Category Name" type="text" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                            </div>
